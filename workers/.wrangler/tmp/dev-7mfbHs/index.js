...
// .wrangler/tmp/bundle-AkAlVL/checked-fetch.js
...
__export(schema_exports, {
  ...
  memories: () => memories,
  messages: () => messages2,
  thoughtTypeEnum: () => thoughtTypeEnum,
  thoughts: () => thoughts
});
var agents, thoughtTypeEnum, thoughts, activityLogs, conversations, messages2, memories;
var init_schema2 = __esm({
  "src/db/schema.ts"() {
    ...
    agents = pgTable("agents", {
      ...
      chatbot_history: json("chatbot_history").$type().default([]),
    ...
    thoughtTypeEnum = pgEnum("thought_type", ["inner_thought", "decision", "conversation"]);
    thoughts = pgTable("thoughts", {
      id: serial("id").primaryKey(),
    ...
    memories = pgTable("memories", {
      ...
      // 在 Cloudflare Workers 中不支持 vector，使用 text 存储
...
async function getAllAgents(db) {
  try {
    ...
    return result.map((agent) => ({
      ...
      currentTask: null,
      // Reset current task on server start
...
async function updateAgentState(db, agentId, updates) {
  try {
    const updateData = {};
    if (updates.x !== void 0) updateData.x = updates.x.toString();
    if (updates.y !== void 0) updateData.y = updates.y.toString();
...
var init_agents = __esm({
  "src/db/services/agents.ts"() {
    ...
    init_drizzle_orm();
    init_schema2();
    __name(getAllAgents, "getAllAgents");
    __name(updateAgentState, "updateAgentState");
    __name(getAgentById, "getAgentById");
  }
});

// .wrangler/tmp/bundle-AkAlVL/middleware-loader.entry.ts
init_checked_fetch();
init_modules_watch_stub();

// .wrangler/tmp/bundle-AkAlVL/middleware-insertion-facade.js
init_checked_fetch();
init_modules_watch_stub();
...
// src/db/services/thoughts.ts
init_checked_fetch();
init_modules_watch_stub();
init_schema2();
async function addThought(db, thought) {
  try {
    const newThought = {
...
__name(addThought, "addThought");
function dbThoughtToFrontend(dbThought) {
  return {
    ...
    metadata: {
      confidence: dbThought.confidence ? dbThought.confidence / 100 : void 0,
      reasoning: dbThought.reasoning || void 0,
      shouldInitiateChat: dbThought.shouldInitiateChat === 1 ? true : void 0,
      emotion: dbThought.emotion || void 0,
      conversationId: dbThought.conversationId || void 0
    }
  };
}
__name(dbThoughtToFrontend, "dbThoughtToFrontend");
...
app.use("*", cors({
  origin: /* @__PURE__ */ __name((origin, c) => {
    ...
    const isAllowed = allowedOrigins.some(
      (allowed) => allowed.includes("*") ? origin.includes(allowed.replace("*.", "")) : origin === allowed
    );
...
app.get("/api/agents", async (c) => {
  try {
    ...
    const agents2 = await getAllAgents(db);
...
var WebSocketSession = class {
  ...
  async fetch(request) {
    ...
    console.log(`\u{1F50C} \u65B0\u7684WebSocket\u8FDE\u63A5\u8BF7\u6C42 - URL: ${request.url}`);
    console.log(`\u{1F4CA} \u5F53\u524D\u4F1A\u8BDD\u6570: ${this.sessions.size}`);
    console.log(`\u{1F4CA} \u5F53\u524D\u5BF9\u8BDD\u6570: ${this.activeConversations.size}`);
  ...
  async handleSession(webSocket) {
    ...
    const agents2 = Array.from(this.agentStates.values());
    this.sendToSocket(webSocket, "init", { agents: agents2, townTime: this.townTime });
  ...
  async handleStopAgentMovement(data) {
    const { agentId } = data;
    const agent = this.agentStates.get(agentId);
    if (!agent) return;
    const updates = {
  ...
  async handleTaskComplete(data, webSocket) {
    const { agentId, status, position } = data;
    const agent = this.agentStates.get(agentId);
    if (!agent) {
    ...
    console.log(`\u{1F4CD} Agent ${agent.name}(${agentId}) \u4EFB\u52A1\u5B8C\u6210: ${agent.status} -> ${updates.status} at (${updates.x}, ${updates.y})`);
  ...
  checkAgentCollision(agentId, position) {
  ...
  async handleAgentCollision(agentId, collidedAgent, updates) {
    ...
    console.log(`\u{1F91D} \u4EE3\u7406\u78B0\u649E: ${agent.name}(${agentId}) \u5728 (${updates.x}, ${updates.y}) \u9047\u5230 ${collidedAgent.name}(${collidedAgent.id}) \u5728 (${collidedAgent.x}, ${collidedAgent.y})`);
    this.broadcast("agentStateUpdate", {
      agentId,
      status: "talking",
      position: { x: updates.x, y: updates.y }
  ...
  async loadAgentsFromDatabase() {
    try {
      ...
      const agents2 = await getAllAgents(this.db);
      console.log(`\u4ECE\u6570\u636E\u5E93\u83B7\u53D6\u5230 ${agents2.length} \u4E2A\u4EE3\u7406:`, agents2.map((a) => `${a.name}(${a.id})`));
      ...
      console.log(`\u2705 \u6210\u529F\u52A0\u8F7D ${agents2.length} \u4E2A\u4EE3\u7406\uFF0C\u5F53\u524D agentStates \u5927\u5C0F: ${this.agentStates.size}`);
    } catch (error) {
      console.error("\u274C \u4ECE\u6570\u636E\u5E93\u52A0\u8F7D\u4EE3\u7406\u5931\u8D25:", error.message || error);
      console.log("\u{1F4E6} \u4F7F\u7528\u9ED8\u8BA4\u4EE3\u7406\u6570\u636E...");
      const defaultAgents = [
  ...
  assignRandomTask(agentId, webSocket) {
    const agent = this.agentStates.get(agentId);
    ...
    console.log(`\u{1F3AF} \u4E3A\u4EE3\u7406 ${agent.name}(${agentId}) \u5206\u914D\u79FB\u52A8\u4EFB\u52A1\u5230 (${targetPosition.x}, ${targetPosition.y})`);
  ...
  isPointInObstacle(x, y, mapConfig) {
    return mapConfig.obstacles.some(
      (obstacle) => x >= obstacle.x && x <= obstacle.x + obstacle.width && y >= obstacle.y && y <= obstacle.y + obstacle.height
    );
  }
  async generateConversationMessages(conversationId, participants) {
          ...
          (async () => {
            ...
            try {
              ...
              try {
                ...
                console.log(`\u{1F4BE} \u5DF2\u4FDD\u5B58 ${speaker} \u7684\u5BF9\u8BDD\u5230 thoughts \u8868`);
              } catch (error) {
                console.error(`\u4FDD\u5B58\u5BF9\u8BDD\u5230 thoughts \u8868\u5931\u8D25:`, error);
              }
              console.log(`\u{1F5E3}\uFE0F  ${speaker}: ${message.content} [${message.emotion}]`);
            } catch (error) {
              console.error(`\u274C \u5BF9\u8BDD\u751F\u6210\u5931\u8D25 (${speaker}):`, error);
              console.log(`\u{1F6AB} \u7531\u4E8EAI\u751F\u6210\u5931\u8D25\uFF0C\u63D0\u524D\u7ED3\u675F\u5BF9\u8BDD ${conversationId}`);
              await this.endConversation(conversationId);
              return;
            }
          })();
        }, (i + 1) * (1e3 + Math.random() * 2e3));
        this.timers.add(timer2);
      }
    } catch (error) {
  ...
  // 获取代理个性信息（从数据库）
  async getAgentPersonalityForServer(agentId) {
    try {
      const { getAgentById: getAgentById2 } = await Promise.resolve().then(() => (init_agents(), agents_exports));
      const agent = await getAgentById2(this.db, agentId);
      if (!agent) {
        return {
          name: `Agent${agentId}`,
          interests: ["\u804A\u5929", "\u4EA4\u6D41", "\u5B66\u4E60"]
      ...
      const result = await this.db.select().from(agents).where(eq(agents.id, agentId)).limit(1);
      const fullAgent = result[0];
      const interests = fullAgent?.tags && Array.isArray(fullAgent.tags) ? fullAgent.tags : ["\u7F16\u7A0B", "\u6280\u672F", "\u4EA4\u6D41"];
      return {
        name: agent.name,
        interests,
  ...
  // 为代理生成消息内容
  async generateMessageForAgent(personality, messageIndex) {
...
\u5F53\u524D\u662F\u7B2C${messageIndex + 1}\u8F6E\u5BF9\u8BDD\u3002`;
    ...
    console.log(`\u{1F916} ${personality.name} AI\u751F\u6210\u5BF9\u8BDD: "${content}" [${emotion}]`);
    return {
      content,
      emotion
    };
  ...
  async endConversation(conversationId) {
    const conversation = this.activeConversations.get(conversationId);
    if (!conversation) return;
    console.log(`\u{1F51A} \u7ED3\u675F\u5BF9\u8BDD: ${conversationId} (${conversation.agent1Name} vs ${conversation.agent2Name})`);
